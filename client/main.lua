...
Citizen.CreateThread(function()
	while RSCore == nil do
		Citizen.Wait(10)
		TriggerEvent("RSCore:GetObject", function(obj)
			RSCore = obj
		end)
	end
	Wait(1000)
...
function SpawnVehicles(vehicles)
	Citizen.CreateThread(function()
		for i = 1, #vehicles, 1 do
			...
			Wait(100)
			--EntityPosition(tempVeh, true)
			SetVehicleHandlingFloat(tempVeh, "CHandlingData", "fInitialDriveForce", 0.0) 
			if vehicles[i].owner ~= PlayerData.citizenid then
...
function SpawnVehicle(vehicleData)
	Citizen.CreateThread(function()
		...
		if vehicleData.owner ~= PlayerData.citizenid then
			SetVehicleDoorsLocked(tempVeh, 2)
		end
		table.insert(LocalVehicles, {
			entity = tempVeh,
...
function RemoveVehicles()
	for k, v in pairs(Config.ParkingLocations) do
		...
		while notFound == false and timeOut < 3000 do
			timeOut = timeOut + 1
			local veh, distance = RSCore.Functions.GetClosestVehicle(tmpLoc)
...
function DeleteNearVehicle(location)
	local veh, distance = RSCore.Functions.GetClosestVehicle(location)
...
function ImpoundVehicle(vehicle)
	for i = 1, #LocalVehicles do
		if vehicle == LocalVehicles[i].entity then
			RSCore.Functions.TriggerCallback("esx_realparking:impoundVehicle", function(callback)
				...
				if storedVehicle ~= false    then
					...
					RSCore.Functions.TriggerCallback("esx_realparking:driveCar", function(callback)
						if callback.status then
							...
							DriveVehicle(carajo) 
							storedVehicle = not storedVehicle
							RSCore.Functions.Notify(carajo.message)
						else
							RSCore.Functions.Notify(carajo.message)
						end
						Wait(1000)
					...
					if veh ~= 0 then
						if IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)) or IsThisModelABicycle(GetEntityModel(veh)) then
							...
							DoScreenFadeOut(250)
							Wait(500)
							RSCore.Functions.TriggerCallback("esx_realparking:saveCar", function(callback)
								if callback.status then
									DeleteVehicle(veh)
							...
							RSCore.Functions.Notify(_U("only_allow_car"))
						end
					end
				end
			end
